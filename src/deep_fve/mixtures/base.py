...
def _kernel_e_step(X, means, cov, ws, xp=cp):
    ...
    log_prob_kernel = cp.ElementwiseKernel(
        name="gmm_log_prob",
    ...
    weighting_kernel = cp.ElementwiseKernel(
    ...
    log_prob_norm = logsumexp(log_prob, axis=1, xp=xp)
...
def _basic_e_step(X, means, cov, ws, xp=np):
    ...
    log_prob_norm = logsumexp(weighted_log_prob, axis=1, xp=xp)
...
class GPUMixin(abc.ABC):
    ...
    def _initialize_parameters(self, X, random_state):
        super(GPUMixin, self)._initialize_parameters(cp.asnumpy(X), random_state)
    ...
    def sample(self, n_samples=1):
        ...
        self.means_, self.covariances_, self.weights_ = map(cp.asnumpy, _save)
